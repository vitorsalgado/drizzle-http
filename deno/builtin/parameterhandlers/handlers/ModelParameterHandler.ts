...
} from "./QueryParameterHandler.ts";
import {
  HeaderParameter,
  HeaderParameterHandler,
} from "./HeaderParameterHandler.ts";
import { QueryNameParameterHandler } from "./QueryNameParameterHandler.ts";
import { FormParameter, FormParameterHandler } from "./FormParameterHandler.ts";
import { BodyParameterHandler } from "./BodyParameterHandler.ts";
import { PathParameter, PathParameterHandler } from "./PathParameterHandler.ts";
import { Mapping, ModelRegistry } from "./ModelRegistry.ts";
...
export class ModelParameter extends Parameter {
  ...
  constructor(
    ...
    public readonly model: Class,
  ) {
    super(index, ModelParameter.Type);
  }
}

export class ModelParameterHandler
  implements ParameterHandler<Record<string, unknown>> {
  constructor(
  ...
  handle(
    ...
    if (hasBodyPart) {
      this.converter.convert(
        this.requestFactory,
        requestValues,
        bodyParts as unknown as BodyType,
      );
    }
  }
}

export class ModelArgumentParameterHandlerFactory
  implements ParameterHandlerFactory<ModelParameter, Record<string, unknown>> {
  static INSTANCE = new ModelArgumentParameterHandlerFactory();

  provide(
    drizzle: Drizzle,
    requestFactory: RequestFactory,
    p: ModelParameter,
  ): ParameterHandler<Record<string, unknown>> | null {
    if (p.type !== ModelParameter.Type) {
      return null;
    }

    const mappings = ModelRegistry.modelMappings().filter((x) =>
      x.model === p.model
    );
      ...
      switch (mapping.to) {
        case "header":
          parameterHandlers.set(
            prop,
            new HeaderParameterHandler(param as HeaderParameter),
          );
          break;
        case "query":
          parameterHandlers.set(
            prop,
            new QueryParameterHandler(param as QueryParameter),
          );
          break;
        case "queryname":
        ...
        case "param":
          parameterHandlers.set(
            prop,
            new PathParameterHandler(param as PathParameter),
          );
          break;
        case "field":
          parameterHandlers.set(
            prop,
            new FormParameterHandler(param as FormParameter),
          );
          break;
        case "body":
          parameterHandlers.set(
