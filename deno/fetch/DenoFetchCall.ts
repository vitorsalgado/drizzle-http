...
import { isAbsolute } from '../internal/index.ts'

export class DenoFetchCall implements Call<Response> {
  ...
  execute(request: HttpRequest): Promise<Response> {
    const url = !isAbsolute(request.url) ? new URL(request.url, this.baseUrl).href : request.url
    ...
    return fetch(url, DenoFetchCall.requestInit(this.requestInit, request, controller.signal)).finally(() =>
      clearTimeout(timer)
    )
  }
}
